...
5. **Language-specific patterns to check** (apply whichever is relevant to the language in scope):
   ...
   - **PostgreSQL**: dynamic SQL via `EXECUTE format()` or `EXECUTE` with unparameterized input, `COPY FROM PROGRAM` executing arbitrary shell commands, `pg_read_file()` / `pg_read_binary_file()` exposing filesystem paths, overly permissive roles (`GRANT ALL ON ALL TABLES TO app_user`), missing row-level security policies on multi-tenant tables, unencrypted sensitive columns (consider `pgcrypto`), `search_path` injection via untrusted schema names, extensions requiring `SUPERUSER` left publicly accessible, missing audit triggers or history tables on financial records, hardcoded passwords in `.sql` scripts or migration files
...
10. **Business logic security (financial applications)**:
    ...
    - Flag use of `Float` or `Double` for monetary values — floating point arithmetic produces rounding errors in financial calculations; require `NUMERIC`/`DECIMAL` with explicit precision and scale in PostgreSQL

...
-- PendingTransaction --
--------------------
CREATE TABLE IF NOT EXISTS int.t_pending_transaction
(
...
(
    ...
    CONSTRAINT fk_account_name_owner FOREIGN KEY (account_name_owner) REFERENCES int.t_account (account_name_owner) ON UPDATE CASCADE
);

---------------
-- Transfer --
---------------
CREATE TABLE IF NOT EXISTS int.t_transfer
(
    ...
    date_updated        TIMESTAMP     DEFAULT PARSEDATETIME('1970-01-01 00:00:00.0', 'yyyy-MM-dd HH:mm:ss.S') NOT NULL,
    date_added          TIMESTAMP     DEFAULT PARSEDATETIME('1970-01-01 00:00:00.0', 'yyyy-MM-dd HH:mm:ss.S') NOT NULL,
    CONSTRAINT fk_source_account FOREIGN KEY (source_account) REFERENCES int.t_account (account_name_owner) ON UPDATE CASCADE,
    CONSTRAINT fk_destination_account FOREIGN KEY (destination_account) REFERENCES int.t_account (account_name_owner) ON UPDATE CASCADE
);
...
(
    medical_provider_id   BIGINT AUTO_INCREMENT PRIMARY KEY,
    provider_name         TEXT UNIQUE                           NOT NULL,
    provider_address      TEXT                                  NULL,
    provider_phone_number TEXT                                  NULL,
...
(
    ...
    date_updated     TIMESTAMP     DEFAULT PARSEDATETIME('1970-01-01 00:00:00.0', 'yyyy-MM-dd HH:mm:ss.S') NOT NULL,
    date_added       TIMESTAMP     DEFAULT PARSEDATETIME('1970-01-01 00:00:00.0', 'yyyy-MM-dd HH:mm:ss.S') NOT NULL,
    CONSTRAINT ck_first_name_lowercase CHECK (first_name = LOWER(first_name)),
    CONSTRAINT ck_last_name_lowercase CHECK (last_name = LOWER(last_name)),
    CONSTRAINT unique_family_member UNIQUE (first_name, last_name, date_of_birth)
);

---------------------
-- Medical Expense --
---------------------
CREATE TABLE IF NOT EXISTS int.t_medical_expense
(
    medical_expense_id   BIGINT AUTO_INCREMENT PRIMARY KEY,
    medical_provider_id  BIGINT                                NOT NULL,
    ...
    out_of_pocket_amount NUMERIC(8, 2) DEFAULT 0.00            NULL,
    active_status        BOOLEAN       DEFAULT TRUE            NOT NULL,
    date_updated         TIMESTAMP     DEFAULT PARSEDATETIME('1970-01-01 00:00:00.0', 'yyyy-MM-dd HH:mm:ss.S') NOT NULL,
    date_added           TIMESTAMP     DEFAULT PARSEDATETIME('1970-01-01 00:00:00.0', 'yyyy-MM-dd HH:mm:ss.S') NOT NULL,
    CONSTRAINT fk_medical_provider FOREIGN KEY (medical_provider_id) REFERENCES int.t_medical_provider (medical_provider_id) ON DELETE CASCADE,
    CONSTRAINT fk_family_member FOREIGN KEY (family_member_id) REFERENCES int.t_family_member (family_member_id) ON DELETE CASCADE,
    CONSTRAINT ck_description_lowercase CHECK (description = LOWER(description)),
    CONSTRAINT unique_medical_expense UNIQUE (medical_provider_id, family_member_id, expense_date, amount, description)
);
